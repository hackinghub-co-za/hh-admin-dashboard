...
-- TryHackMe username, job title/employment status, specialty, job readiness)
-- and self-edit their own entry. member_profiles is otherwise locked to
-- admins-only for every operation (FOR ALL USING is_admin(...)) and holds
-- genuinely sensitive columns (money_owed, monthly_remuneration, phone,
...
-- revised twice more (a security fix restricting get_member_directory() to
-- approved members and locking update_my_directory_profile() to UPDATE-only,
-- then the TryHackMe username field, then hiding unnamed profiles). Rather
-- than leave those revisions spread across later files - which would silently
-- regress this file back to an outdated, less secure state if it were ever
-- re-run on its own - every change has been folded back in here. The
-- once-separate 014_tryhackme_username.sql and
...
-- Write, scoped to only the caller's own row and only these 8 public-facing
