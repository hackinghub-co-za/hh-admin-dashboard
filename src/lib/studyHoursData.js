 ...
 * Study Hours first. */
export async function logStudySession(track, plannedMinutes) {
  const { error } = await supabase.rpc('log_study_session', { p_track: track || null, p_planned_minutes: plannedMinutes });
  if (error) throw error;
}
